...
/**
 * Testing mixed but unordered reads and writes in Hyperledger Fabric (sending reads and writes at same time)
 */
 ...
 const csvWriterReads = createCsvWriter({
   path: 'read-write_test_reads_async.csv',
   header: [
 ...
 const csvWriterWrites = createCsvWriter({
	 path: 'read-write_test_writes_async.csv',
	 header: [
	   {id: 'startTime', title: 'T'},
	   {id: 'responseTime', title: 'Response Time'}
	 ]
	 ...
	 try {
		 ...
		 try {
			 ...
			 for (let i = 0; i < numIterations; i++){
				...
				await Promise.all([
					measurePromise(() => contract.submitTransaction('UpdateAsset', 'asset1', 'blue', (i / 2).toString(), 'Tomoko', '350'))
				 		.then((response) => {
				 	...
				 	measurePromise(() => contract.submitTransaction('UpdateAsset', 'asset1', 'blue', (i / 3).toString(), 'Tomoko', '350'))
				 		.then((response) => {
					 		output_data_writes.push({startTime: response.start, responseTime: response.elapsed});
					 		totalTimeWrites += response.elapsed;
					 		console.log(`*** WRITE Result - UpdateAsset:  ${prettyJSONString(response.value.toString())}`);
				 		}),
					measurePromise(() => contract.submitTransaction('GetAllAssets'))
				 		.then((response) => {
			 ...
			 var averageTimeReads = totalTimeReads / (numIterations * 2);
			 var averageTimeWrites = totalTimeWrites / (numIterations * 2);
 
			 console.log(`Average time for reads: ${averageTimeReads}`);
